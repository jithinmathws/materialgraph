...
def test_rank_path_scores_full_alkali_substitution_path():
    ...
    assert result["score_breakdown"]["transition_plausibility"] == 20.0
...
def test_rank_path_scores_two_hop_path_efficiency_lower():
    ...
    assert result["scientific_usefulness_score"] == 81.0
    assert result["score_breakdown"]["framework_preservation"] == 30.0
    assert result["score_breakdown"]["objective_alignment"] == 25.0
    assert result["score_breakdown"]["transition_plausibility"] == 18.5
    assert result["score_breakdown"]["material_quality"] == 0.0
